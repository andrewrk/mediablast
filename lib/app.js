var assert, express, path, temp, fs, cors, sse, Jobs, bodyParser;
...
function create(settings){
  ...
  app.post('/', [bodyParser, cors], function(req, resp){
    ...
    if (user_template = settings.json.templates[templateId]) {
      template = (ref$ = import$({}, user_template), ref$.id = templateId, ref$);
  ...
  function createWithTemplate(req, resp, templateId, template){
    ...
    temp_files = res$;
    context = (ref$ = req.body, ref$.makeTemp = makeTemp, ref$);
